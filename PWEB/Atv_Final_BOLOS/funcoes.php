...
function enviar_msg($id_bolo,$nome,$data,$numero,$email,$entrega,$gostou,$mudar,$tema,$ref,$topo,$geral){
    $tipo_resposta = 'Em andamento';
    $status_msg = 'Nova' ;
    $data_finalizacao = 'Em andamento';
    $venda = 'Não';
    require "conexao.php";
    $sql = $conn->prepare("INSERT INTO mensagem(`id_bolo`,`nome`,`data`,`telefone`,
                        `email`,`gostou`,`mudar`,`tema`,`ref`,`topo`,`geral`,`entrega`
                        ,`tipo_resposta`,`status_msg`,`data_finalizacao`,`venda`) 
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    $sql->bind_param("ississssssssssss",$id_bolo,$nome,$data,$numero,$email,$gostou,
